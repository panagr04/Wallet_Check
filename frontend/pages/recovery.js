...
import login from '../public/lottie/register.json'

import { useRouter } from 'next/router';
import {generateOtp,updatePass} from "../api/services"
...
export default function Home() {
	...
	const genOTP = async (data) =>{
		setOpt(true) 

		let result = await generateOtp(data)
		
		setOpt(false)
		console.log(result)
		result?.data?.status && setOtpRef(result?.data?.otp_ref)
	}
  const onSubmit = (data) => {
		data.otp_ref=otpRef
		let result = updatePass(data,router)
	}
  ...
  <div className="flex-col justify-around hero-content lg:flex-row w-full">
    ...
    <div className="card flex-shrink-0 w-full max-w-sm shadow-2xl bg-base-100">
		...
		<h1 className=" mb-5 text-5xl font-bold text-neutral-content">Recovery</h1>
		<Link passHref href="/login" className="btn btn-secondary  z-10">
  ...
  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
      ...
      <div className="card-body">
			<form onSubmit={handleSubmit(genOTP)}>
          ...
          <button
            type="submit"
            className={`btn btn-sm btn-outline btn-primary ${otp ? "loading" : ""}`}
          >Generate OTP
						</button>
        </div>								
				...
				{otpRef && <form onSubmit={handleSubmit(onSubmit)}>
				<div className="form-control">
          <label className="label flex justify-between items-center flex-wrap">
            <span className="label-text">One Time Password</span>
						{errors.otp?.type === 'required' && <div className="badge badge-error">Required</div>}
						{errors.otp?.message && <div className="badge badge-error">{errors.otp.message}</div>}
          </label>
          <input
            type="number"
						{...register("otp",{ required: true,maxLength:{value:6,message:"OTP can only be 6 DIGIT"}})}
            placeholder="******"
            className="input input-bordered"
          />
        </div>
          ...
          <label className="label flex justify-between items-center flex-wrap">
            <span className="label-text">New Password</span>
						{errors.newpass?.type === 'required' && <div className="badge badge-error">Required</div>}
          </label>
          <input
            type="password"
          ...
          <label className="label flex justify-between items-center flex-wrap">
            <span className="label-text">Username  </span>
						{errors.username?.type === 'required' && <div className="badge badge-error">Required</div>}
          </label>
          <input
            type="text"
            placeholder="User7777"
		...
		<h1 className="mb-5 text-3xl font-bold">⚠ Oops!<br/> Looks like you have lost<br/> your way let us assist you</h1>
